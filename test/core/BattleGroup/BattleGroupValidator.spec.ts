...
describe('BattleGroupValidator', () => {
	const instance = new BattleGroupValidator()
	const MAX_BATTLE_GROUP_SIZE = 5
	const sizedBattleGroupType = new SizedBattleGroupType(
		BattleGroupType.Line,
		GroupSizing.size(1, MAX_BATTLE_GROUP_SIZE)
	)
	...
	describe('example with single tonnage class', () => {
		...
		test('have minimal such groups', () => {
			group = new BattleGroup(sizedBattleGroupType, new Array(MIN_GROUPS_NUM).fill(GROUP_1), extractor)
			const errors = instance.validate(group)
			expect(errors).toHaveLength(0)
		})

		test('have groups more than allowed', () => {
			group = new BattleGroup(sizedBattleGroupType, new Array(MAX_GROUPS_NUM + 1).fill(GROUP_1), extractor)
			const errors = instance.validate(group)
			expect(errors).toHaveLength(1)
			expect(errors[0]).toContain(
				`There is ${MAX_GROUPS_NUM +
					1} of ${EXAMPLE_TONNAGE_CLASS} group(s), but only ${MAX_GROUPS_NUM} allowed`
			)
		})
	})
	...
	test('Check for forbidden tonnage', () => {
		...
		const EXAMPLE_FORBIDDEN_TONNAGE_UNIT = UnitFactory({ tonnage: EXAMPLE_FORBIDDEN_TONNAGE_CLASS })
		const GROUP_1 = Group.build(EXAMPLE_FORBIDDEN_TONNAGE_UNIT, 1)

		const EXAMPLE_ALLOWED_TONNAGE_UNIT = UnitFactory({ tonnage: EXAMPLE_ALLOWED_TONNAGE_CLASS })
		const GROUP_2 = Group.build(EXAMPLE_ALLOWED_TONNAGE_UNIT, 1)

		group = new BattleGroup(sizedBattleGroupType, [GROUP_1, GROUP_2], extractor)
		const errors = instance.validate(group)
		expect(errors).toHaveLength(1)
		expect(errors[0]).toContain(
	...
	test('Internal validation: group validator called', () => {
		...
		group = new BattleGroup(sizedBattleGroupType, [GROUP_1, GROUP_2, GROUP_3], extractor)
