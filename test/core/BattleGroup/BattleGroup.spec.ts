...
describe('BattleGroup spec', () => {
	...
	test('size return number of groups', () => {
		...
		instance = new BattleGroup(sizedBattleGroupType, [GROUP_1, GROUP_2, GROUP_3], mkExtractor(1, 3))
	...
	describe('addGroup method', () => {
		...
		test('throw error if battle group is full', () => {
			...
			const executor = (): BattleGroup => instance.addGroup(new Group(UNIT_1))
			expect(executor).toThrow()

			instance = new BattleGroup(sizedBattleGroupType, [GROUP_1, GROUP_2], mkExtractor(1, 3))
		...
		test('throw error if there is more groups with such tonnage than allowed', () => {
			...
			instance = new BattleGroup(sizedBattleGroupType, [new Group(LIGHT_UNIT)], mkExtractor(0, 1))
			const executor = (): BattleGroup => instance.addGroup(new Group(LIGHT_UNIT))
			expect(executor).toThrow()
	...
	describe('removeGroup', () => {
		test('throw error if provided group does not exists in battle group', () => {
			instance = new BattleGroup(sizedBattleGroupType, [GROUP_1, GROUP_2], mkExtractor(1, 3))
			const executor = (): BattleGroup => instance.removeGroup(GROUP_3)
			expect(executor).toThrow()
		...
		test('throw error if trying to remove required group', () => {
			instance = new BattleGroup(sizedBattleGroupType, [GROUP_1], mkExtractor(1, 3))
			const executor = (): BattleGroup => instance.removeGroup(GROUP_1)
			expect(executor).toThrow()
		})
	})
})
